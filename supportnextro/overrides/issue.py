...
def login_to_site(url, username, password):
    try:
        ...
        if login_res.json().get("message") != "Logged In":
            safe_log("Login Failed", f"Could not login to {url}")
            return None, None
...
def upload_file(session, csrf_token, site_url, file_name, file_content, doctype, docname):
    try:
        ...
        if result.get("message"):
            return result["message"].get("file_url")
        safe_log("File Upload Failed", str(result))
        return None
    except Exception:
...
def on_update(doc, method):
    ...
    if doc.get("custom_by_ticket") == 1 and doc.get("custom_site_name") and doc.get("custom_issue_id"):
        reverse_sync_to_source(doc)
...
def sync_to_ticket_portal(doc):
    try:
        ...
        if not result.get("data"):
            safe_log("Ticket Sync Failed", str(result))
            return

        remote_doc_name = result["data"].get("name")
        safe_log("Ticket Sync Created", f"Remote issue: {remote_doc_name}")
...
def reverse_sync_to_source(doc):
    try:
        ...
        try:
            ...
            result = {"response": update_res.text[:1000]}

        if not result.get("data"):
            safe_log("Reverse Sync Failed", str(result)[:5000])
        else:
            safe_log("Reverse Sync Success", f"{custom_issue_id} updated on {source_site}")
